...
const Associates = () => {
  ...
  function applySortFilter(array, comparator, query, status, department, office) {
    let filtered = array;
    ...
    if (department) {
      filtered = filtered.filter((_user) => _user.Department === department);
    }

    // 3. Filter by Office
    if (office) {
      filtered = filtered.filter((_user) => _user.Office === office);
    }

    // 4. Filter by Query (Name, Title, Department, Start Date)
    if (query) {
      const q = query.toLowerCase();
      filtered = filtered.filter((_user) => {
        return (
          (_user.FirstName &&
            _user.FirstName.toLowerCase().indexOf(q) !== -1) ||
          (_user.LastName &&
            _user.LastName.toLowerCase().indexOf(q) !== -1) ||
          (_user.Title && _user.Title.toLowerCase().indexOf(q) !== -1) ||
          (_user.Department &&
            _user.Department.toLowerCase().indexOf(q) !== -1) ||
          (_user.StartDate &&
            moment(_user.StartDate.toDate()).format("DD-MM-yyyy").indexOf(q) !==
            -1)
        );
      });
    }
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell align="right">
                                <UserMoreMenu id={id} userName={`${FirstName} ${LastName}`} />
                              </TableCell>
                            </TableRow>
                          );
                        })}
