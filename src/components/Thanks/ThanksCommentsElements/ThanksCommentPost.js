...
const ThanksCommentPost = ({ count, thanksId, fromUser, toUser, refreshSocialData }) => {
  ...
  const postComment = async () => {
    try {
      const response = await fetch(`http://localhost:8081/thanks/${thanksId}/comment`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          associate_id: parseInt(fromUser.id),
